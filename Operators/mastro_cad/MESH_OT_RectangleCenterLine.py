...
class MESH_OT_MaStroCad_RectangleCenterLine(RectMixin, bpy.types.Operator):
    """Draw a rectangle by clicking the center line then the width.

    Click 1 — center of first edge
    Click 2 — center of opposite edge (defines direction and length)
    Mouse/type — half-width perpendicular to center line (H toggles half/full)
    """
    bl_idname  = "mastrocad.rectangle_centerline"
    bl_label   = "Rectangle (Center Line)"
    bl_options = {'REGISTER', 'UNDO'}

    _draw_handle   = None
    _p1            = None   # center of first edge
    _p2            = None   # center of second edge (confirmed on 2nd click)
    _mouse_pt      = None   # live mouse for p2 preview
    _plane_normal  = None
    _base_dir      = None
    _perp          = None
    _rect_corners  = None
    ...
    _number_input  = ""
    _half_mode     = True   # True = type half-width, False = type full width
    ...
    def _draw_preview(self, context):
        try:
            ...
            MESH_OT_MaStroCad_RectangleCenterLine._gc_cleanup()
            return
    ...
    def _update_header(self, context):
        if self._p1 is None:
            context.area.header_text_set(
                "Rectangle  |  click center line start  |  ESC cancel")
        elif self._p2 is None:
            context.area.header_text_set(
                "Rectangle  |  click center line end  |  ESC cancel")
        else:
            half = "ON" if self._half_mode else "OFF"
            val  = self._number_input if self._number_input else "move mouse"
            context.area.header_text_set(
                f"Rectangle  |  Width or move mouse: {val}"
    ...
    def _update_preview(self, context, mouse_2d):
        ...
        self._rect_corners = [p1 - self._perp * hw,
                               p2 - self._perp * hw,
                               p2 + self._perp * hw,
                               p1 + self._perp * hw]
    ...
    def modal(self, context, event):
        ...
        if event.type in {'MOUSEMOVE', 'INBETWEEN_MOUSEMOVE'}:
            ...
            if self._p1 is None:
                ...
                self._snap_hit = snapped
                self._mouse_pt  = snapped if snapped is not None else raw_pt
        ...
        elif event.value == 'PRESS' and self._p2 is not None:
            if event.type == 'H':

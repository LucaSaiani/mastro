...
def add_block_attributes(obj):
    ...
    for a in block_attribute_set:
        if a["attr"] not in mesh.attributes:
            if a["attr_domain"] == ("EDGE","FACE"):
                ...
                mesh.attributes.new(name=a["attr"], type=a["attr_type"], domain='FACE')
            else:
    ...
    for a in block_attribute_set:
        ...
        if a["attr"] == "mastro_typology_id":
            ...
            val = bpy.context.scene.mastro_floor_name_list_index
        elif a["attr"] == "mastro_number_of_storeys":
            val = numberOfStoreys
        elif a["attr"] == "mastro_wall_id":
            val = bpy.context.scene.mastro_wall_name_list_index
        else:
            val = a["attr_default"]
        
        if 'POINT' in a["attr_domain"]:
            for vert in mesh.vertices:
                attr[vert.index].value = val
        if 'EDGE' in a["attr_domain"]:
            for edge in mesh.edges:
                attr[edge.index].value = val
